...
/*--------------------------------------------------------------------*/
 /* the main matrix over GF(2) in sparse format */          
la_col_t *create_matrix(int64_t size) {

   int64_t i;
   la_col_t *matrix;

   matrix = (la_col_t*)malloc((size)*sizeof(la_col_t));
...
void reduce_matrix(uint32_t extra_rels, uint32_t *nrows, uint32_t *ncols, la_col_t *cols) {
	...
	do {
		...
		if (reduced_cols > reduced_rows + extra_rels) {
			for (i = reduced_rows + extra_rels;
					i < reduced_cols; i++) {
			...
			reduced_cols = reduced_rows + extra_rels;
		}
...
static void mul_64xN_Nx64(uint64_t *x, uint64_t *y,
			   uint64_t *c, uint64_t *xy, uint32_t n) {
...
/*-------------------------------------------------------------------*/
static uint32_t find_nonsingular_sub(uint64_t *t, uint32_t *s, 
...
/*-------------------------------------------------------------------*/
void mul_MxN_Nx64(uint32_t vsize, uint32_t dense_rows,
		uint32_t ncols, la_col_t *A,
...
/*-------------------------------------------------------------------*/
void mul_trans_MxN_Nx64(uint32_t dense_rows, uint32_t ncols,
...
/*-----------------------------------------------------------------------*/
static void transpose_vector(uint32_t ncols, uint64_t *v, uint64_t **trans) {
...
/*-----------------------------------------------------------------------*/
void combine_cols(uint32_t ncols, 
	...
	uint32_t i, j, k, bitpos, col, col_words, num_deps;
	uint64_t mask;
...
/*-----------------------------------------------------------------------*/
uint64_t * block_lanczos(uint32_t nrows, uint32_t dense_rows, 
   uint32_t ncols, la_col_t *B) {
	...
	scratch = (uint64_t *)malloc(MAX(vsize, 256 * 8) * sizeof(uint64_t));
	...
	for (i = 0; i < n; i++)
		v[0][i] = (uint64_t) rand() + ((uint64_t) rand() << 32);
	...
	printf("lanczos halted after %u iterations\n", iter);
   ...
   free(vnext);
	...
	if (i < ncols) {
		printf("lanczos error: dependencies don't work %u\n",i);
		abort();
	}
