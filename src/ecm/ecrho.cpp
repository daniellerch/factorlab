...
// {{{ ecm_rho_2()
EC::Point F(const EC::Point &P, const long B1, const EC::Point &A)
{
   //static std::unordered_map<long, NTL::ZZ> pa_map(B1);
   static std::unordered_map<long, NTL::ZZ> pa_map;
   NTL::PrimeSeq seq;
   seq.reset(0);
   NTL::ZZ pa;
   EC::Point Q = P;
   ...
   for(long p=seq.next(); p<=B1; p=seq.next())
   {
      if(pa_map.find(p)==pa_map.end())
      {
         ...
         //pa_map[p]=pa;
         pa_map.insert(std::pair<long,NTL::ZZ>(p,pa));
      }
      else
         pa = pa_map[p];
...
NTL::ZZ ecm_rho_2(const EC::Point &P, long B1, long max_it)
{
   NTL::ZZ m1, m2, x;
   EC::Point U=P, Uc=P;
   EC::Point V=P, Vc=P;
   ...
   for(long l=0; l<max_it; l++)
   {
      ...
      */
      ...
      NTL::ZZ_p f = NTL::sqr(Uc.X*Vc.Z - Vc.X*Uc.Z);
      NTL::GCD(x, NTL::ZZ_p::modulus(), NTL::rep(f));
      if(1<x && x<NTL::ZZ_p::modulus())
      {
...
NTL::ZZ ecrho(NTL::ZZ &n, long B1, long B2)
{
   ...
   for (;;curves++)
   {
      ...
      NTL::ZZ factor = ecm_rho_2(Q, B1, 500);
